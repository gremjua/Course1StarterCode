...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {
		...
		//ellipse(50, 225, 5, 5);
		fill(color(255, 0, 0));
		triangle(50-4, 100+4, 50, 100-4, 50+4, 100+4);
		fill(color(255, 255, 255));
		ellipse(50, 125, 10, 10);
		rect(50-5, 150-5, 10, 10);

		fill(color(0,0,255));
		ellipse(50, 200, 10, 10);	//shallow
		fill(color(255,255,0));
		ellipse(50, 225, 10, 10);	//intermediate
		fill(color(255,0,0));
		ellipse(50, 250, 10, 10);	//deep
	...
	private void printQuakes()
	{
		...
		for (Marker m : quakeMarkers) {
			...
			if (em.isOnLand()) {
				...
				if(countMap.containsKey(country)) {
					countMap.put(country, countMap.get(country) + 1);	//increase number of quakes in that country
				}
				else {
					countMap.put(country, 1);	//first appearance of the country, so the count is 1
				}
			}
			else {
				oceanQuakes++;	//increase number of ocean quakes
			}
		}

		Map<String,Integer> treeMap = new TreeMap<String,Integer>(countMap);	//sort countries
