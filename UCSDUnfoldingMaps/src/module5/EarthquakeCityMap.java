...
public class EarthquakeCityMap extends PApplet {
		...
		for(Marker m : markers) {
			if(m.isInside(map, mouseX, mouseY) && lastClicked == null) {
				...
				if(m instanceof EarthquakeMarker){
					processEarthquakeClicked((EarthquakeMarker)m);
				}
				else if (m instanceof CityMarker){
					processCityClicked((CityMarker)m);
				}
	...
	private void processEarthquakeClicked(EarthquakeMarker m) {
		double radius = m.threatCircle();
		for(Marker c : cityMarkers) {
			if(m.getDistanceTo(c.getLocation()) > radius) {	//hide cities beyond the threat circle
				c.setHidden(true);
			}
		}
	...
	private void processCityClicked(CityMarker m) {
		for(Marker q : quakeMarkers) {
			if(m.getDistanceTo(q.getLocation()) > ((EarthquakeMarker)q).threatCircle()) {	//hide cities beyond the threat circle
				q.setHidden(true);
			}
		}
